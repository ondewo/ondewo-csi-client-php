...
[ondewo-csi-api](https://github.com/ondewo/ondewo-csi-api) repository by the
...
// `package ondewo.csi;` becomes `Ondewo\Csi`, and a service `Foo` becomes
// `FooClient` (grpc_php_plugin's default class suffix). Browse src/Ondewo/Csi for
// the services and messages your pinned API version actually declares.
$client = new \Ondewo\Csi\ExampleServiceClient(
...
$request = new \Ondewo\Csi\ExampleRequest();
...
`ONDEWO_CSI_VERSION` at the top of the `Makefile` is the single source of truth and **must match
the ONDEWO CSI API in major and minor version**. `make update_composer_version` propagates it into
