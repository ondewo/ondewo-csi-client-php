...
The client is published on [Packagist](https://packagist.org/packages/ondewo/csi-client-php) as
**`ondewo/csi-client-php`** and is installed like any other composer package:

```bash
composer require ondewo/csi-client-php
```

The client version tracks the ONDEWO CSI API in major and minor version, so pin the minor series that
...
* **`ext-grpc` is a hard requirement**, not an optional extra — every generated `<Service>Client`
  ...
  (see [Requirements](#requirements) above); `composer require ondewo/csi-client-php --ignore-platform-req=ext-grpc`
...
* **There is no build step.** Packagist serves the tree of a git tag verbatim and the generated stubs are
  ...
  `composer require`, `vendor/ondewo/csi-client-php/src/` already holds every stub and the autoloader is
  generated by composer itself. Nothing needs docker, protoc or the `ondewo-csi-api` submodule.
...
1. **Submit the package once.** Log in to [packagist.org](https://packagist.org/) with the ONDEWO account and
   use *Submit* with the repository URL `https://github.com/ondewo/ondewo-csi-client-php`. Packagist reads
