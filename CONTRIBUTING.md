...
| `ondewo-csi-api`, `ondewo-proto-compiler` | their own repositories | as submodule commits |
...
1. Bump `ONDEWO_CSI_API_GIT_BRANCH` (and, for a new compiler release,
...
Releases are cut from the `Makefile`, never by hand: bump `ONDEWO_CSI_VERSION` (it must match the
ONDEWO CSI API in major and minor version), add a `RELEASE.md` entry under a
`## Release ONDEWO CSI PHP Client <version>` heading followed by the `*****` separator the release
